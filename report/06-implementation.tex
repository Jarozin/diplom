...
\begin{itemize}
    ...
    \item \texttt{ahp\_method.py} --- класс \texttt{AHPMethod} (реализация полного МАИ для критериев: матрица попарных сравнений, вычисление весов через геометрическое среднее, проверка согласованности) и класс \texttt{AHPRecommendationRanker} (ранжирование рекомендаций на основе весов критериев с последующей нормализацией).
...
\item \textbf{10. Сравнение реализованных методов МАИ} --- запуск сравнительного анализа классического МАИ и модифицированного МАИ. 
...
\item сохранение изменений в новые файлы (в дерикторию \texttt{debug}) с привязкой к текущему профилю;
...
При выборе этого пункта программа предлагает пользователю выбрать JSON-файл из дериктории \texttt{research/} для сравнения.
...
Результаты сравнения могут быть сохранены в файлы в дериктории \texttt{debug/} для последующего анализа.
